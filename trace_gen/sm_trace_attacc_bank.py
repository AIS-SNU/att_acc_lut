...
def run_gemv(n,m, k, packing_factor, trace_file_name):
  total_cmd = []
  valid_channel = n_channel
  big_offset = pow(2, 20) #
  new_k = math.ceil(k / packing_factor) # k is packed!
  ...
  for ba_idx in range(n_bank): # number of partitions
    for col_idx in range(math.ceil(n*new_k/ n_bank / n_mac)):
  ...
  for n_idx in range(n):
    for m_idx in range(math.ceil( m / n_pch / n_rank / n_bg)):# 16 
      for k_idx in range(math.ceil(new_k / n_bank / n_mac)): # 2
        idx = k_idx + m_idx * math.ceil(dhead / n_bank / n_mac) 
      ...
      ## A output element is generated for every m_idx
      # todo(SM): output 읽어오는 부분인데 score cal 하는 부분 그대로 가져온거라
      # todo(SM) : 좀 더 정확하게 하려면 수정이 필요할 수 있음
      if m_idx % 16 == 15 or n_idx == math.ceil(m / n_pch / n_rank / n_bg) - 1:
...
def main():
  ...
  parser.add_argument("-o", "--output", type=str, default="gemv_attacc_bank.trace", 
                      help="output path")
